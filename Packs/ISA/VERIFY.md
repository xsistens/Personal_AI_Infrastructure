...
```bash
CLAUDE_DIR="$HOME/.claude"
head -1 "$CLAUDE_DIR/skills/ISA/SKILL.md" | grep -q "^---" && echo "OK frontmatter delimited" || echo "ERROR missing frontmatter"
grep -q "^name:" "$CLAUDE_DIR/skills/ISA/SKILL.md" && echo "OK has name" || echo "ERROR missing name"
grep -q "^description:" "$CLAUDE_DIR/skills/ISA/SKILL.md" && echo "OK has description" || echo "ERROR missing description"
```
...
```markdown
## ISA Installation Verification

- [ ] Skill directory exists at ~/.claude/skills/ISA/
